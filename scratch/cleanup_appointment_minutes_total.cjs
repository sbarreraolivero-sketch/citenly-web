...
async function run() {
    console.log('--- Iniciando Limpieza TOTAL de Minutos ---')
    ...
    console.log(`Encontradas ${appointments.length} citas en total para analizar.`)
    ...
    for (const appt of appointments) {
        ...
        if (![0, 15, 30, 45].includes(mins) || secs !== 0 || ms !== 0) {
            ...
            console.log(`[LIMPIEZA] ${appt.patient_name}: ${appt.appointment_date} -> ${newISO}`)
