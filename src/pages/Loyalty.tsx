...
export default function Loyalty() {
    ...
    return (
        <div className="space-y-6 animate-fade-in pb-20">
            {/* Banner — Marketing */}
            <div className="bg-gradient-to-br from-violet-500 to-violet-700 rounded-2xl overflow-hidden shadow-soft-md mb-8">
                <div className="p-6 sm:p-8">
                    <div className="flex items-start justify-between gap-4">
                        <div className="flex-1">
                            <p className="text-xs font-black uppercase tracking-widest text-violet-200 mb-2">Marketing</p>
                            <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-white">Fidelización</h1>
                            <p className="text-sm text-violet-100/80 font-light mt-1">Premia a tus mejores pacientes y fomenta el crecimiento orgánico de forma automática.</p>
                        </div>
                        <div className="flex items-center gap-4 flex-wrap justify-end">
                            <div className="hidden sm:flex items-center gap-3">
                                <div className="bg-white/15 rounded-xl px-4 py-2.5 text-center border border-white/20">
                                    <p className="text-xs font-black uppercase tracking-widest text-violet-200 mb-0.5">{settings?.loyalty_points_name || 'Saldo'}</p>
                                    <p className="text-xl font-black text-white">{stats.totalPointsDist.toLocaleString()}</p>
                                </div>
                                <div className="bg-white/15 rounded-xl px-4 py-2.5 text-center border border-white/20">
                                    <p className="text-xs font-black uppercase tracking-widest text-violet-200 mb-0.5">Referidores</p>
                                    <p className="text-xl font-black text-white">{stats.totalReferrals}</p>
                                </div>
                                <div className="bg-white/15 rounded-xl px-4 py-2.5 text-center border border-white/20">
                                    <p className="text-xs font-black uppercase tracking-widest text-violet-200 mb-0.5">Recompensas</p>
                                    <p className="text-xl font-black text-white">{stats.activeAlerts}</p>
                                </div>
                            </div>
